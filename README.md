...
Turn is the latest request; Session is the mean TTFT and aggregate decode TPS across the session. When a request has no trustworthy decode source (for example a stream that a proxy held and released in one burst), its TPS slot shows the end-to-end rate dimmed with an `e2e` tag, e.g. `TPS 57.3 e2e`, instead of a decode number that would really be delivery speed.
...
- Footer decode TPS, in priority order: provider `Server-Timing` decode metrics; a gateway's `*-sent-at` epoch header to the last delta on our clock (`tpsSource: anchored`, accepted only when the header falls inside our own request window so clock skew is bounded by a round trip); AIPerf-style event decode `(output - 1) / (latency - TTFT)`. Falls back to dimmed `N e2e` for buffered or single-event responses.
...
- `serverTtftSec`: `sent-at - received-at` from gateway headers, the server-side time to first token. `headerDelaySec`: how long after `sent-at` the response headers reached us; anything beyond network latency is a proxy holding the stream. `anchoredSec`: the sent-at-to-last-delta window.
- `headerLagFromDateSec`: the same lag estimated from the HTTP `Date` header at 1 s resolution, for origins that send no `sent-at`. On a `buffered: true` request, near zero means the origin held the response; several seconds means the origin had already sent headers and this process saw them late (event loop stalled). Diagnostic only; `Date` is too coarse to anchor decode TPS. Session decode excludes requests without a trustworthy decode source.
